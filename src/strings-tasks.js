...
function isPalindrome(str) {
  const cleaniser = str.replace(/[^a-zA-Z0-9]/g, '').toLowerCase();
  return cleaniser === cleaniser.split('').reverse().join('');
}
...
function findLongestWord(sentence) {
  ...
  return words.reduce((longest, currentW) => {
    const cleanedW = currentW.replace(/[^a-zA-Z]/g, '');
    return cleanedW.length > longest.length ? cleanedW : longest;
  }, '');
}
...
function invertCase(str) {
  ...
  const invertChar = character.map((char) =>
    char === char.toUpperCase() ? char.toLowerCase() : char.toUpperCase()
  );
  const result = invertChar.join('');
  return result;
}
